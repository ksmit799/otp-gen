...
export default class Mapping implements DCMapping {{
    ...
    public getClasses(): string[] {{
        return Object.keys(Mapping.id2class);
    }}
    
    public getClassForId(id: number): any {{
        return Mapping.id2class[id];
    }}
    
    public getFunctionForId(id: number): any {{
        return Mapping.id2fn[id];
    }}
    
    public getFunctionNameForId(id: number): string {{
        return Mapping.fnid2name[id];
    }}
    
    public getClassNameForFunctionId(id: number): string {{
        return Mapping.fnid2clsname[id];
    }}
    
    public getRemoteInterface(clsName: string): any {{
        return Mapping.cls2rmccls[clsName];
    }}
    
    public getOwnerInitFunction(clsName: string): any {{
        return Mapping.cls2owninitfn[clsName];
    }}
    
    public getInitFunction(clsName: string): any {{
        return Mapping.cls2initfn[clsName];
    }}
