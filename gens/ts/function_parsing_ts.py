    ...
    def _gen_buffer(self):
        ...
        for name, dc_class in self.dcLoader.dclasses_by_name.items():
            ...
            for i in range(dc_class.get_num_fields()):
                ...
                static_out += f"\tpublic static call_{name}_{field.getName()}(dc_interface: I{name}, dgi: DatagramIterator): void {{\n"
                ...
                if molecular_field:
                    ...
                    for k in range(molecular_field.getNumAtomics()):
                        ...
                        static_out += f"\t\tFunctionParsing.call_{name}_{atomic_field.getName()}(dc_interface, dgi);\n"
                    ...
                    for k in range(atomic_field.getNumElements()):
                        ...
                        if elem_simple:
                            ...
                            params += (
                                f"dgi.read{get_formatted_subatomic_type(elem_type)}(), "
                            )
                        elif elem_array:
                            pass
                    ...
                    if elem_simple:
                        elem_type = elem_simple.getType()
                        static_out += f"\t\tdc_interface.{field.getName()}(dgi.read{get_formatted_subatomic_type(elem_type)}());\n"
                    else:
                        pass
